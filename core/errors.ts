export type ErrorKind = 'bad_key' | 'rate_limited' | 'quota_exhausted' | 'network' | 'unknown';
...
export function toSDKError(error: unknown): GeminiSDKError {
  ...
  return new GeminiSDKError('unknown', `An unknown Gemini SDK error occurred: ${message}`, error);
}
